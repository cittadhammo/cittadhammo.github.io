...
<head>
  ...
  <style>
    ...
    svg {{
      display: block;
      width: {svg_width_mm}mm !important;
      height: {svg_height_mm}mm !important;
...
def rasterize_svg(svg_path, png_output_path, page_w_mm, page_h_mm, background_color):
    px_w = mm_to_px(page_w_mm)
    px_h = mm_to_px(page_h_mm)
    bg = "--background" if background_color == "black" else ""
    subprocess.run([
        "rsvg-convert",
        "--width", str(px_w),
        "--height", str(px_h),
...
def process_vector(name, label, license_config=None):
    ...
    try:
        ...
        rasterize_svg(source_svg, png_path, page_w_mm, page_h_mm, background_color)
