...
DEFAULT_DPI = 300
MIN_DPI = 160
# Keep giant formats (A0/2A0) manageable for downstream processing.
MAX_LONG_EDGE_PX = 12000
MAX_PIXELS = 100_000_000
ADAPTIVE_DPI = os.getenv("PNG_ADAPTIVE_DPI", "false").lower() == "true"
MM_PER_INCH = 25.4
...
print(
    "PNG mode:",
    "adaptive" if ADAPTIVE_DPI else "fixed",
    f"(default_dpi={DEFAULT_DPI}, adaptive_enabled={ADAPTIVE_DPI})"
)
...
def choose_png_dpi(page_w_mm, page_h_mm):
    if not ADAPTIVE_DPI:
        return float(DEFAULT_DPI)

    w_in = page_w_mm / MM_PER_INCH
    h_in = page_h_mm / MM_PER_INCH
    long_edge_in = max(w_in, h_in)
    area_in2 = max(w_in * h_in, 1e-9)

    dpi_from_edge = MAX_LONG_EDGE_PX / long_edge_in
    dpi_from_pixels = math.sqrt(MAX_PIXELS / area_in2)

    dpi = min(DEFAULT_DPI, dpi_from_edge, dpi_from_pixels)
    dpi = max(MIN_DPI, dpi)
    return dpi
...
def convert_pdf_to_png(pdf_file, png_output_path, page_w_mm, page_h_mm, dpi):
    doc = fitz.open(str(pdf_file))
    page = doc.load_page(0)

    target_px_w = mm_to_px(page_w_mm, dpi)
    target_px_h = mm_to_px(page_h_mm, dpi)
...
for svg_path in svg_dir.glob("*.svg"):
    ...
    try:
        ...
        png_dpi = choose_png_dpi(page_w_mm, page_h_mm)
        target_px_w = mm_to_px(page_w_mm, png_dpi)
        target_px_h = mm_to_px(page_h_mm, png_dpi)
        print(
            f"PNG render settings for {base_name}: "
            f"dpi={png_dpi:.1f}, size={target_px_w}x{target_px_h}, "
            f"pixels={(target_px_w * target_px_h):,}"
        )
        convert_pdf_to_png(pdf_path, png_path, page_w_mm, page_h_mm, png_dpi)
